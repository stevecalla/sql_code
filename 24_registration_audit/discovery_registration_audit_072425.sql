...
SELECT 
    "#3 membership period count last two calendar days" AS query_label,
...
WHERE 1 = 1
    AND mp.updated_at >= TIMESTAMP(DATE(CONVERT_TZ(UTC_TIMESTAMP(), 'UTC', 'America/Denver')) - INTERVAL 1 DAY)
...
SELECT
    "#4 RA WITH RAMA PRICE PAID AND NO MEMBERSHIP ID" AS query_label
    , ra.id AS id_ra
...
WHERE 1 = 1
    ...
    AND ra.created_at >="2026-05-01" -- reviewing njuko launch
HAVING 1 = 1
...
WITH test AS (
    SELECT
        ...
        , rc.name AS name_rc
        , rama.price_paid AS price_paid_rama
        , CASE WHEN rama.price_paid IS NULL THEN 0 ELSE 1 END AS has_price_paid_rama -- i think rama price paid null is validation
        , ra.created_at as created_at_ra
        , ra.processed_at as processed_at_ra
        , mp.id
    ...
    WHERE 1 = 1
        -- AND DATE(ra.created_at) >= "2025-07-16" -- TS issue
        AND DATE(ra.created_at) >= "2026-05-01" -- njuko launch
    ...
    FROM test
    WHERE 1 = 1 
		-- AND processed_at_ra IS NULL
        AND registration_company_id_ra = 33 -- njuko
    GROUP BY 1, 2, 3, 4, 5, 6
    ORDER BY 4, 3, 2;

    SELECT 
        "#5a PROCESSED OR NOT?" AS query_label,
        is_processed_over_1_hour,
        has_membership_period_id,
