/* ============================================================
Q1 - Summary: Row-level reconciliation between ORIGINAL and COPY_042126
============================================================ */
SELECT 
...
FROM (
    SELECT
        'Q1 - Summary: missing_in_copy_042126' AS query_label,
        COUNT(*) AS row_count
    FROM usat_sales_db.all_runsignup_data_raw_affiliate_urls o
    LEFT JOIN usat_sales_db.all_runsignup_data_raw_affiliate_urls_copy_042126 c
    ...
    SELECT
        'Q1 - Summary: new_in_copy_042126' AS query_label,
        COUNT(*) AS row_count
    FROM usat_sales_db.all_runsignup_data_raw_affiliate_urls_copy_042126 c
    LEFT JOIN usat_sales_db.all_runsignup_data_raw_affiliate_urls o
...
/* ============================================================
Q1b - Summary: Compare registration_url_final_rule counts between ORIGINAL and COPY_042126
Purpose:
...
FROM (
    SELECT
        ...
        COALESCE(c.row_count, 0) AS copy_042126_row_count,
    ...
    LEFT JOIN (
        SELECT
            registration_url_final_rule,
            COUNT(*) AS row_count
        FROM usat_sales_db.all_runsignup_data_raw_affiliate_urls_copy_042126
        WHERE registration_url_final IS NOT NULL
        GROUP BY registration_url_final_rule WITH ROLLUP
    ) c
...
/* ============================================================
Q1c - Summary: Compare race_next_year_date counts between ORIGINAL and COPY_042126
Purpose:
...
/* ============================================================
Q1d - Summary: Compare usat_match_year_date counts between ORIGINAL and COPY_042126
Purpose:
...
/* ============================================================
Q2 - Detail: Rows in ORIGINAL but missing in COPY_042126
============================================================ */
SELECT 
    ROW_NUMBER() OVER () AS row_num,
    COUNT(*) OVER () AS total_rows,
    'Q2 - Detail: missing_in_copy_042126' AS query_label,
    o.*
FROM usat_sales_db.all_runsignup_data_raw_affiliate_urls o
LEFT JOIN usat_sales_db.all_runsignup_data_raw_affiliate_urls_copy_042126 c
    ON o.race_id = c.race_id
...
/* ============================================================
Q3 - Detail: Rows in COPY_042126 but not in ORIGINAL
============================================================ */
SELECT 
    ROW_NUMBER() OVER () AS row_num,
    COUNT(*) OVER () AS total_rows,
    'Q3 - Detail: new_in_copy_042126' AS query_label,
    c.*
FROM usat_sales_db.all_runsignup_data_raw_affiliate_urls_copy_042126 c
LEFT JOIN usat_sales_db.all_runsignup_data_raw_affiliate_urls o
    ON c.race_id = o.race_id
...
SELECT 
    ROW_NUMBER() OVER () AS row_num,
    COUNT(*) OVER () AS total_rows,
    'Q4 - Detail: field_level_differences_vs_copy_042126' AS query_label,
    o.race_id,
...
FROM usat_sales_db.all_runsignup_data_raw_affiliate_urls o
JOIN usat_sales_db.all_runsignup_data_raw_affiliate_urls_copy_042126 c
    ON o.race_id = c.race_id
WHERE 1 = 1
    AND (
...
    ROW_NUMBER() OVER () AS row_num,
    COUNT(*) OVER () AS total_rows,
    'Q5 - Detail: checksum_differences_vs_copy_042126' AS query_label,
    o.race_id
FROM usat_sales_db.all_runsignup_data_raw_affiliate_urls o
JOIN usat_sales_db.all_runsignup_data_raw_affiliate_urls_copy_042126 c
    ON o.race_id = c.race_id
