...
-- actual_membership_fee_6 CALCULATION
    ...
    SELECT 
        ...
        MAX(
            CASE
                ...
                -- todo: scalla added this rule because bronze 6 was being priced at 23
                WHEN rama.price_paid IN (6.00)      THEN 6 
                WHEN rama.price_paid IN (13.00)     THEN 13
                WHEN rama.price_paid IN (18.00)     THEN 18
                WHEN rama.price_paid IN (23.00)     THEN 23
...
one_day_sales_actual_member_fee AS (
    ...
    WHERE
        ...
        AND (CASE 
            WHEN membership_types.id IN (5, 46, 47, 72, 97, 100, 115, 118) THEN 1
...
--         ) AS max_membership_period_id, -- get the max membership_period.id for each duplicate record
--         DATE(membership_periods.created_at),
--         YEAR(membership_periods.purchased_on) AS purchase_on_year_membership_periods, -- added to summarize by year
--         membership_periods.starts, -- one_day
--         membership_periods.ends, -- one_day
--         membership_periods.membership_type_id,
--         events.sanctioning_event_id,
--         membership_applications.payment_type,
--         membership_periods.origin_flag,
--         membership_applications.race_type_id, -- one_day
--         membership_applications.distance_type_id, -- one_day
--         order_products.order_id,
--         membership_applications.confirmation_code,
...
-- COACHES SALES ACTUAL MEMBER FEE
-- coaches_sales_actual_member_fee AS (
--     SELECT 
--         DISTINCT -- DISTINCT = ensure that the combination of all selected columns in the result set is unique
--         members.member_number AS member_number_members,
--         ( SELECT MAX(mp.id)
...
--     FORMAT(COUNT(DISTINCT max_membership_period_id), 0) as purchases
-- FROM one_day_sales_actual_member_fee
-- GROUP BY purchased_on_year_membership_periods
...
-- -- FROM coaches_sales_actual_member_fee
...
-- SELECT
--     purchased_on_year_membership_periods,
--     CAST(member_number_members AS UNSIGNED) AS member_number_members,
...
--     FORMAT(SUM(max_membership_fee_6), 0)
-- FROM one_day_sales_actual_member_fee
-- GROUP BY member_number_members
-- ORDER BY member_number_members, max_membership_period_id
;

-- SECTION: STEP #5 - add purchase data for one day, annual, club
-- SECTION: STEP #6 - add revenue data for one day, annual, club
-- SECTION: STEP #7 - merge all data together with all fields