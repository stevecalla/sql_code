...
-- Error Code: 1146. Table 'usat_match.profiles' doesn't exist

SET @membership_period_ends = '2008-01-01'; -- Used for membership_periods.ends
-- SET @start_date = '2025-01-01 00:00:00'; -- Used for membership_periods.purchased_on
-- SET @end_date = '2025-12-31 23:59:59'; -- Used for membership_periods.purchased_on

SET @start_date = '2025-02-01 00:00:00'; -- Used for membership_periods.purchased_on
SET @end_date = '2025-02-28 23:59:59'; -- Used for membership_periods.purchased_on

-- Section 1: cte source_2_type
-- Purpose: Takes information from all membership applications, registration audit, and order_products.
-- JAN2025CHANGES:
    -- 1. Changed the CASE WHEN statement to be a left join instead with a smaller case statement. Joins in vapor.registration_companies
    -- 2. Removed single membership that was excluded. Membership was inactivated in the database

WITH source_2_type AS (
    SELECT 
        membership_periods.id 'id_membership_periods',
        ...
        -- JAN2025CHANGE Commented out CASE statement
        /*
        CASE -- Vapor table: registration_companies id 28 = Alpha Win
            ...
            WHEN registration_audit.registration_company_id = 28 THEN 'Alpha Win'
            WHEN registration_audit.registration_company_id IS NOT NULL THEN registration_audit.registration_company_id
            WHEN order_products.order_id IS NOT NULL THEN 'Braintree' -- Will not be accurate once we switch bulk upload process as there will be an order product for those.
...
-- SELECT * FROM source_2_type
-- order by id_membership_periods

,

-- Section 2: KOZ Acception
-- Purpose: KOZ was a series of event that had occurred where we had issues with the payments. Will be assigned either $10 or $15 based on membership type
...
-- SELECT * FROM actual_membership_fee_6
-- ORDER BY  id_membership_periods
...
-- SELECT * FROM actual_membership_fee_6_rule
-- ORDER BY id_membership_periods
...
-- SELECT * FROM new_member_category_6
-- ORDER BY id_membership_periods

,

-- Section 5: one_day_sales_actual_member_fee
-- Purpose: Removes some duplicate items
-- JAN2025CHANGES:
    -- 1. Commented out section limiting to one-day or annual sales.
...
one_day_sales_actual_member_fee AS (
    ...
    WHERE 1 = 1
        ...
        /* AND (
            ...
            ELSE 0 END 
            ) = 1 */ -- COMMENTED THIS SECTION OUT TO PULL ONE DAY & ANNUAL
        AND 
            (
            CASE
            ...
            WHEN 
                CASE
                    WHEN order_products.cart_label IS NOT NULL THEN 'Membership System/RTAV Classic' -- Exists in order products
                    WHEN registration_audit_membership_application.price_paid IS NOT NULL THEN 'RTAV Batch' -- Exists in Registration Audit
                    WHEN membership_types.name IS NOT NULL THEN 'Other' -- Has a membership type
                END IS NULL
                AND CASE
                        WHEN members.deleted_at IS NOT NULL OR -- 
                            membership_periods.deleted_at IS NOT NULL OR 
...
-- Section 7: add_all_fields (Extracting from final CTE and any relevant vapor tables)
-- Purpose: Pull all data together for export. Create some additional custom fields
-- JAN2025CHANGES: No changes
...
SELECT 
    ...
    *
FROM add_all_fields
WHERE origin_flag_mp IN ('ADMIN_BULK_UPLOADER')
;