...
, aggregated_participation_stats AS (
    SELECT
        ...
        -- -- [Region Metrics]
        -- COUNT(DISTINCT region_name) AS count_of_race_regions_distinct, -- todo:
        -- GROUP_CONCAT(DISTINCT region_name ORDER BY start_date_races ASC) AS race_regions_distinct, -- todo:
        -- SUBSTRING_INDEX(GROUP_CONCAT(DISTINCT region_name ORDER BY region_name DESC), ',', 1) AS most_recent_region_name, -- todo:
...
-- WHERE 1 = 1
--     AND profile_id = 489329 
--     AND most_recent_region_name = "Central"
-- GROUP BY 1, 2, 3, 4, 5
-- ORDER BY 4, 2, 1, 3, 5
-- LIMIT 1000
-- ;
