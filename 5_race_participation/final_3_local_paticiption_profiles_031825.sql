...
WITH participant_race_count AS (
    SELECT 
        ...
        , SUBSTRING_INDEX(GROUP_CONCAT(name_race_type ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_name_race_type
        -- EVENT DISTANCE
        , GROUP_CONCAT(DISTINCT name_distance_types ORDER BY start_date_year_races ASC) AS name_distance_types_distinct
        , SUBSTRING_INDEX(GROUP_CONCAT(name_distance_types ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_name_distance_types
        -- EVENT TYPE
        , GROUP_CONCAT(DISTINCT name_event_type ORDER BY start_date_year_races ASC) AS name_event_type_distinct
        , SUBSTRING_INDEX(GROUP_CONCAT(name_event_type ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_name_event_type
        -- EVENT NAME
        , GROUP_CONCAT(DISTINCT name_events_rr ORDER BY start_date_year_races ASC) AS name_events_distinct
        , SUBSTRING_INDEX(GROUP_CONCAT(name_events_rr ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_name_events
        -- EVENT ZIP CODE
        , GROUP_CONCAT(DISTINCT zip_events ORDER BY start_date_year_races ASC) AS zip_events_distinct
        , SUBSTRING_INDEX(GROUP_CONCAT(zip_events ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_zip_events
        -- IS IRONMAN
        , GROUP_CONCAT(DISTINCT is_ironman ORDER BY start_date_year_races ASC) AS is_ironman_distinct
        , SUBSTRING_INDEX(GROUP_CONCAT(is_ironman ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_is_ironman
        ...
        , SUBSTRING_INDEX(GROUP_CONCAT(gender_code ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_gender_code
        -- AGE
        , GROUP_CONCAT(DISTINCT age ORDER BY start_date_year_races ASC) AS age_distinct
        , SUBSTRING_INDEX(GROUP_CONCAT(age ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_age
        ...
        , SUBSTRING_INDEX(GROUP_CONCAT(id_membership_periods_sa ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_id_membership_period_sa
        -- MEMBERSHIP TYPES
        ...
        , SUBSTRING_INDEX(GROUP_CONCAT(real_membership_types_sa ORDER BY purchased_on_date_adjusted_mp ASC),',', 1) AS least_recent_membership_type
        , SUBSTRING_INDEX(GROUP_CONCAT(real_membership_types_sa ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_membership_type
        ...
        , SUBSTRING_INDEX(GROUP_CONCAT(new_member_category_6_sa ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_new_member_category_6_sa
        ...
        , SUBSTRING_INDEX(GROUP_CONCAT(member_created_at_category ORDER BY purchased_on_date_adjusted_mp ASC),',', 1) AS least_recent_member_created_at_category
        , SUBSTRING_INDEX(GROUP_CONCAT(member_created_at_category ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_member_created_at_category
        -- starts_mp
        , GROUP_CONCAT(DISTINCT starts_mp ORDER BY start_date_year_races ASC) AS starts_mp_distinct
        , SUBSTRING_INDEX(GROUP_CONCAT(starts_mp ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_starts_mp
        -- ends_mp
        , GROUP_CONCAT(DISTINCT ends_mp ORDER BY start_date_year_races ASC) AS ends_mp_distinct
        , SUBSTRING_INDEX(GROUP_CONCAT(ends_mp ORDER BY purchased_on_date_adjusted_mp DESC),',', 1) AS most_recent_ends_mp
...
participant_race_count_average AS (
    ...
    WHERE 1 = 1
        AND start_year_least_recent > 2022
        AND most_recent_member_created_at_category = 'created_year'
        -- AND least_recent_member_created_at_category = 'created_year'
