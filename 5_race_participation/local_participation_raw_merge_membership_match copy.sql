...
WITH participation AS (
	...
	WHERE 1 = 1
        ...
        -- AND id_sanctioning_events = '309562' -- double checking against Sam's report with 639 participants & 602 active memberships
        -- AND id_profile_rr = 1086344 -- first sample to test the match of membership record with race results
...
, membership_sales_data AS (
	...
	ORDER BY starts_mp ASC -- AND purchased_on_year_adjusted_mp = 2025
    -- LIMIT 10
)

-- SELECT * FROM membership_sales_data;
, merge_member_sales_with_active_race AS (
    SELECT 
        s.*
		, p.id_profile_rr
		, p.id_rr
    ...
    FROM membership_sales_data AS s
        LEFT JOIN participation AS p ON p.id_profile_rr = s.id_profiles
        AND s.starts_mp <= p.start_date_races
        AND s.ends_mp >= p.start_date_races
	-- ORDER BY s.starts_mp ASC
    -- ORDER BY p.id_profile_rr, p.start_date_races, p.id_race_rr, s.id_membership_periods_sa
)

-- SELECT * FROM merge_member_sales_with_active_race;
-- SELECT * FROM merge_member_sales_with_active_race WHERE NOT(name_events_m <=> name_events_p); -- returns true if the values differ (including if one is NULL and the other isn’t)
SELECT id_profile_rr, COUNT(*), COUNT(DISTINCT name_events_m), COUNT(DISTINCT name_events_p) FROM merge_member_sales_with_active_race WHERE NOT(name_events_m <=> name_events_p) GROUP BY 1;
