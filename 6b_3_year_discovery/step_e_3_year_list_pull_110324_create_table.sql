...
CREATE TABLE relaunch_3_year_110324 AS
    SELECT
        *
    FROM (
        -- CTE or subquery for purchase history goes here
        WITH members_with_ends_in_2024 AS (
            ...
            WHERE new_member_category_6_sa IN (@member_category)  -- Filter by member category
            AND YEAR(ends_mp) = @year_2024  -- ends in 2024
        ),
        purchase_history AS (
            SELECT mw.member_number_members_sa,
                sa.id_profiles,
                ...
                COUNT(mw.member_number_members_sa) OVER (PARTITION BY mw.member_number_members_sa) AS member_purchase_count,
                CASE
                    WHEN sa.ends_mp = MAX(sa.ends_mp) OVER (PARTITION BY mw.member_number_members_sa ORDER BY sa.ends_mp DESC) THEN 1
                    ELSE 0
                END AS is_max_end_after_2024_end
            ...
            SELECT
                ...
                COUNT(id_profiles) AS purchase_count
                -- GROUP_CONCAT(new_member_category_6_sa) AS member_categories
            FROM purchase_history
            GROUP BY member_number_members_sa, id_profiles
            HAVING YEAR(max_end_mp_sa) < 2025
            ORDER BY member_number_members_sa, id_profiles
        )
