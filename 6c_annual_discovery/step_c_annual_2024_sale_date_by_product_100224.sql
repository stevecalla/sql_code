...
-- SECTION: #3) - ROLLUP = WHAT IS THE PRODUCT PURCHASED PRIOR TO THE 3-YEAR 2023 PURCHASE?
    WITH members_with_purchase_in_2023 AS (
        ...
        FROM all_membership_sales_data_2015_left
        WHERE   
            new_member_category_6_sa IN (@member_category_1year, @member_category_gold, @member_category_silver)  -- Filter by member category
        ...
        SELECT 
            ...
            COUNT(DISTINCT CASE WHEN YEAR(purchased_on_adjusted_mp_sa) < @year_2024 THEN member_number_members_sa END) AS 'sale_<2024',
            COUNT(DISTINCT CASE WHEN YEAR(purchased_on_adjusted_mp_sa) = @year_2024 THEN member_number_members_sa END) AS 'sale_2024',
