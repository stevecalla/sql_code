...
SET @prior_year_date = CURDATE() - INTERVAL 1 YEAR;

WITH current_year_data AS (
    SELECT 
        purchased_on_date_adjusted_mp,
        DAYOFWEEK(purchased_on_date_adjusted_mp) AS day_of_week,
    ...
    WHERE 
        ...
        AND purchased_on_date_adjusted_mp < DATE_ADD(DATE_FORMAT(@current_year_date, '%Y-%m-01'), INTERVAL 1 MONTH)
...
prior_year_data AS (
    ...
    WHERE 
        ...
        AND purchased_on_date_adjusted_mp < DATE_ADD(DATE_FORMAT(@prior_year_date, '%Y-%m-01'), INTERVAL 1 MONTH) + INTERVAL 7 DAY -- to allow for calendar comparison into next month
    GROUP BY purchased_on_date_adjusted_mp
),
comparison_data AS (
    SELECT 
    ...
    FROM current_year_data
		LEFT JOIN prior_year_data ON current_year_data.day_of_week = prior_year_data.day_of_week
		AND WEEKOFYEAR(current_year_data.purchased_on_date_adjusted_mp) = WEEKOFYEAR(prior_year_data.prior_year_date)
	WHERE
		current_year_data.purchased_on_date_adjusted_mp < @current_year_date
)
SELECT 
    metric_name,
    CONCAT(
...
FROM (
    ...
    SELECT 
        ...
        current_year_date,
        prior_year_date,
        CONCAT('$', FORMAT(current_revenue, 0)) AS current_value,
        CONCAT('$', FORMAT(prior_revenue, 0)) AS prior_value,
    ...
    SELECT 
        ...
        current_year_date,
        prior_year_date,
        FORMAT(current_units, 0) AS current_value,
        FORMAT(prior_units, 0) AS prior_value,
    ...
    SELECT 
        ...
        current_year_date,
        prior_year_date,
        CONCAT('$', FORMAT(current_revenue / NULLIF(current_units, 0), 2)) AS current_value,
        CONCAT('$', FORMAT(prior_revenue / NULLIF(prior_units, 0), 2)) AS prior_value,
    ...
    SELECT 
        ...
        CONCAT('$', FORMAT(SUM(current_revenue), 0)) AS current_value,
        CONCAT('$', FORMAT(SUM(prior_revenue), 0)) AS prior_value,
    ...
    SELECT 
        ...
        'Grand Total' AS current_year_date,
        'Grand Total' AS prior_year_date,
        FORMAT(SUM(current_units), 0) AS current_value,
        FORMAT(SUM(prior_units), 0) AS prior_value,
    ...
    SELECT 
        ...
        'Grand Total' AS current_year_date,
        'Grand Total' AS prior_year_date,
        CONCAT('$', FORMAT(SUM(current_revenue) / NULLIF(SUM(current_units), 0), 2)) AS current_value,
        CONCAT('$', FORMAT(SUM(prior_revenue) / NULLIF(SUM(prior_units), 0), 2)) AS prior_value,
