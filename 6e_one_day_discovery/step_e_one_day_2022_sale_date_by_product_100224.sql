...
-- SECTION: #3) - ROLLUP = WHAT IS THE PRODUCT PURCHASED PRIOR TO THE 3-YEAR 2022 PURCHASE?
    WITH members_with_purchase_in_2022 AS (
        SELECT 
        ...
        WHERE   
            ...
            AND purchased_on_year_adjusted_mp IN (@year_2022) -- purchased in 2022
        )
        
        -- SELECT COUNT(DISTINCT(member_number_members_sa)) AS unique_member_count, COUNT(member_number_members_sa) AS sales_units FROM members_with_purchase_in_2022;
        -- SELECT * FROM members_with_purchase_in_2022 WHERE member_number_members_sa IN (893515390); -- TIME ZONE ISSUE; ADJUSTED ALL DATES BACK TO MTN
        ...
        , purchase_history AS (
            SELECT
                ...
                mw.ends_mp AS ends_mp_mw, -- 2022 ends date
            ...
            FROM members_with_purchase_in_2022 AS mw
                LEFT JOIN all_membership_sales_data_2015_left AS sa ON mw.member_number_members_sa = sa.member_number_members_sa
            WHERE 
                sa.purchased_on_adjusted_mp <= mw.purchased_on_adjusted_mp -- filters for purchases with purchase date less than the 2022 3-year purchase
        ...
        -- WHERE 
        --     m.rn = 1  -- Only take the most recent purchase prior to the 3-year ending in 2022
        --     -- AND m.ends_mp_sa < '2022-01-01'  -- Ensure the end date is prior to the start of 2022
        ...
        SELECT 
            ...
            COUNT(DISTINCT CASE WHEN YEAR(purchased_on_adjusted_mp_sa) < @year_2022 THEN member_number_members_sa END) AS 'sale_<2022',
            COUNT(DISTINCT CASE WHEN YEAR(purchased_on_adjusted_mp_sa) = @year_2022 THEN member_number_members_sa END) AS 'sale_2022',
            COUNT(DISTINCT CASE WHEN YEAR(purchased_on_adjusted_mp_sa) > @year_2022 THEN member_number_members_sa END) AS 'Other',
...
-- SECTION: #4) - ROLLUP = WHAT IS THE PRODUCT PURCHASED PRIOR TO THE 3-YEAR 2022 PURCHASE BY NUMBER OF PURCHASES?
-- SAME AS QUERY ABOVE EXCEPT LAST SELECT STATEMENT
    WITH members_with_purchase_in_2022 AS (
        ...
        WHERE   
            new_member_category_6_sa IN (@member_bronze_a, @member_bronze_b, @member_bronze_c, @member_bronze_d, @member_bronze_e, @member_bronze_distance_upgrade, @member_bronze_ao, @member_club, @member_one_day_a) -- Filter by member category
            AND purchased_on_year_adjusted_mp IN (@year_2022)                    -- Only include members with end date in 2022
        )
        ...
        , purchase_history AS (
            ...
            FROM members_with_purchase_in_2022 AS mw
                LEFT JOIN all_membership_sales_data_2015_left AS sa ON mw.member_number_members_sa = sa.member_number_members_sa
            WHERE sa.purchased_on_adjusted_mp <= mw.purchased_on_adjusted_mp -- filters for purchases with purchase date less than the 2022 3-year purchase
            ORDER BY mw.member_number_members_sa, sa.purchased_on_adjusted_mp
        )
