...
-- SECTION: #3) - ROLLUP = WHAT IS THE PRODUCT PURCHASED PRIOR TO THE 3-YEAR 2021 PURCHASE?
    WITH members_with_purchase_in_2021 AS (
        SELECT 
        ...
        WHERE   
            ...
            AND purchased_on_year_adjusted_mp IN (@year_2021) -- purchased in 2021
        )
        
        -- SELECT COUNT(DISTINCT(member_number_members_sa)) AS unique_member_count, COUNT(member_number_members_sa) AS sales_units FROM members_with_purchase_in_2021;
        -- SELECT * FROM members_with_purchase_in_2021 WHERE member_number_members_sa IN (893515390); -- TIME ZONE ISSUE; ADJUSTED ALL DATES BACK TO MTN
        ...
        , purchase_history AS (
            SELECT
                ...
                mw.ends_mp AS ends_mp_mw, -- 2021 ends date
            ...
            FROM members_with_purchase_in_2021 AS mw
                LEFT JOIN all_membership_sales_data_2015_left AS sa ON mw.member_number_members_sa = sa.member_number_members_sa
            WHERE 
                sa.purchased_on_adjusted_mp <= mw.purchased_on_adjusted_mp -- filters for purchases with purchase date less than the 2021 3-year purchase
        ...
        -- WHERE 
        --     m.rn = 1  -- Only take the most recent purchase prior to the 3-year ending in 2021
        --     -- AND m.ends_mp_sa < '2021-01-01'  -- Ensure the end date is prior to the start of 2021
...
-- SECTION: #4) - ROLLUP = WHAT IS THE PRODUCT PURCHASED PRIOR TO THE 3-YEAR 2021 PURCHASE BY NUMBER OF PURCHASES?
-- SAME AS QUERY ABOVE EXCEPT LAST SELECT STATEMENT
    WITH members_with_purchase_in_2021 AS (
        ...
        WHERE   
            new_member_category_6_sa IN (@member_bronze_a, @member_bronze_b, @member_bronze_c, @member_bronze_d, @member_bronze_e, @member_bronze_distance_upgrade, @member_bronze_ao, @member_club, @member_one_day_a) -- Filter by member category
            AND purchased_on_year_adjusted_mp IN (@year_2021)                    -- Only include members with end date in 2021
        )
        ...
        , purchase_history AS (
            ...
            FROM members_with_purchase_in_2021 AS mw
                LEFT JOIN all_membership_sales_data_2015_left AS sa ON mw.member_number_members_sa = sa.member_number_members_sa
            WHERE sa.purchased_on_adjusted_mp <= mw.purchased_on_adjusted_mp -- filters for purchases with purchase date less than the 2021 3-year purchase
            ORDER BY mw.member_number_members_sa, sa.purchased_on_adjusted_mp
        )
