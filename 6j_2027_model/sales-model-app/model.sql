...
-- MODEL YEARS / PERIOD
    ...
    SET @this_year = 2026;
    SET @next_year = 2027;
    SET @actual_through_month = 8; -- 1=Jan ... 12=Dec
...
-- CREATE ACTUAL VS GOAL DATA
    ...
    DROP TABLE IF EXISTS sales_model_2027_new;
...
-- CREATE SALES MODEL 2027
    ...
    CREATE TABLE sales_model_2027_new 
        WITH sales_actuals AS (
            SELECT
                ...
                CASE WHEN MONTH(common_purchased_on_date_adjusted) = @actual_through_month THEN 1 ELSE 0 END AS is_current_month,
                CASE WHEN MONTH(common_purchased_on_date_adjusted) <= @actual_through_month THEN 1 ELSE 0 END AS is_year_to_date,
                CASE WHEN MONTH(common_purchased_on_date_adjusted) <= @actual_through_month THEN 1 ELSE 0 END AS is_ytd_before_current_month,
        ...
        sales_goals AS (
            SELECT
                ...
                CASE WHEN purchased_on_month_adjusted_mp = @actual_through_month THEN 1 ELSE 0 END AS is_current_month,
                CASE WHEN purchased_on_month_adjusted_mp <= @actual_through_month THEN 1 ELSE 0 END AS is_year_to_date,
                CASE WHEN purchased_on_month_adjusted_mp <= @actual_through_month THEN 1 ELSE 0 END AS is_ytd_before_current_month,
            ...
            SELECT
                ...
                CASE WHEN sa.month_actual = @actual_through_month THEN 1 ELSE 0 END AS is_current_month,
                CASE WHEN sa.month_actual <= @actual_through_month THEN 1 ELSE 0 END AS is_year_to_date,
                CASE WHEN sa.month_actual <= @actual_through_month THEN 1 ELSE 0 END AS is_ytd_before_current_month,
...
-- DROP OLD / RENAME 2027 TO OLD / RENAME NEW TO 2027
DROP TABLE IF EXISTS sales_model_2027_old;
RENAME TABLE
    sales_model_2027 TO sales_model_2027_old,
    sales_model_2027_new TO sales_model_2027;
DROP TABLE sales_model_2027_old;
