...
-- >>> BUSINESS GOAL LEVERS (MVP): annual goals allocated using this year non-bulk seasonality / mix >>>
    ...
    -- PRICE
    SET @lever_price_pct_change = 0.0;
    ...
    SET @lever_repeat_units_incremental = ROUND(@lever_repeat_members_base * @lever_repeat_members_pct_change, 0);
    ...
    -- NOTE:
    -- new members = allocated across all non-bulk categories using this year mix / seasonality
    -- repeat + win-back = allocated to adult_annual using this year adult_annual mix / seasonality
    ...
    CREATE TABLE sales_model_2027 AS
        ...
        business_lever_shares AS (
            SELECT
                ...
                -- NEW MEMBERS: use overall non-bulk month/category mix
                GREATEST(
                    COALESCE(b.sales_units_this_year_estimate_nonbulk, 0)
                    - COALESCE(b.sales_units_next_year_goal_post_race_base, 0)
                , 0)
                ...
                , 0) AS lever_new_member_allocation_share,

                -- REPEAT / WIN-BACK / UPGRADE: use adult_annual month/category mix
                CASE
        ...
        business_levers AS (
            SELECT
                ...
                -- VOLUME — annual goals allocated to month/category
                @lever_new_member_units_incremental
                    * COALESCE(bls.lever_new_member_allocation_share, 0) AS lever_new_member_units_incremental,

                @lever_repeat_units_incremental
                    * COALESCE(bls.lever_adult_annual_allocation_share, 0) AS lever_repeat_units_incremental,

                @lever_winback_units_incremental
                    * COALESCE(bls.lever_adult_annual_allocation_share, 0) AS lever_winback_units_incremental,
...
SELECT
    ...
    COALESCE(type_goal, 'Grand Total') AS type_goal,
    "Units" AS metric,
    ROUND(SUM(CASE WHEN month_goal = 1  THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `1`,
    ROUND(SUM(CASE WHEN month_goal = 2  THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `2`,
    ROUND(SUM(CASE WHEN month_goal = 3  THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `3`,
    ROUND(SUM(CASE WHEN month_goal = 4  THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `4`,
    ROUND(SUM(CASE WHEN month_goal = 5  THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `5`,
    ROUND(SUM(CASE WHEN month_goal = 6  THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `6`,
    ROUND(SUM(CASE WHEN month_goal = 7  THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `7`,
    ROUND(SUM(CASE WHEN month_goal = 8  THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `8`,
    ROUND(SUM(CASE WHEN month_goal = 9  THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `9`,
    ROUND(SUM(CASE WHEN month_goal = 10 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `10`,
    ROUND(SUM(CASE WHEN month_goal = 11 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `11`,
    ROUND(SUM(CASE WHEN month_goal = 12 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END), 0) AS `12`,
    ROUND(SUM(sales_units_this_year_estimate_nonbulk), 0) AS Grand_Total
FROM sales_model_2027
GROUP BY type_goal WITH ROLLUP
...
SELECT
    "3_this_year_sales_estimate_nonbulk" AS query_label,
    COALESCE(type_goal, 'Grand Total') AS type_goal,
    "Effective Price" AS metric,
    ROUND(
        SUM(CASE WHEN month_goal = 1 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 1 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 1 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `1`,
    ROUND(
        SUM(CASE WHEN month_goal = 2 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 2 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 2 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `2`,
    ROUND(
        SUM(CASE WHEN month_goal = 3 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 3 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 3 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `3`,
    ROUND(
        SUM(CASE WHEN month_goal = 4 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 4 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 4 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `4`,
    ROUND(
        SUM(CASE WHEN month_goal = 5 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 5 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 5 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `5`,
    ROUND(
        SUM(CASE WHEN month_goal = 6 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 6 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 6 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `6`,
    ROUND(
        SUM(CASE WHEN month_goal = 7 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 7 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 7 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `7`,
    ROUND(
        SUM(CASE WHEN month_goal = 8 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 8 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 8 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `8`,
    ROUND(
        SUM(CASE WHEN month_goal = 9 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 9 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 9 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `9`,
    ROUND(
        SUM(CASE WHEN month_goal = 10 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 10 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 10 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `10`,
    ROUND(
        SUM(CASE WHEN month_goal = 11 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 11 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 11 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `11`,
    ROUND(
        SUM(CASE WHEN month_goal = 12 THEN sales_rev_this_year_estimate_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 12 THEN sales_units_this_year_estimate_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 12 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `12`,
    ROUND(
        SUM(sales_rev_this_year_estimate_nonbulk)
        /
        NULLIF(
            SUM(sales_units_this_year_estimate_nonbulk)
            - SUM(sales_units_next_year_goal_post_race)
        , 0)
    , 2) AS Grand_Total
FROM sales_model_2027
GROUP BY type_goal WITH ROLLUP
...
SELECT
    ...
    COALESCE(type_goal, 'Grand Total') AS type_goal,
    "Units" AS metric,
    ROUND(SUM(CASE WHEN month_goal = 1  THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `1`,
    ROUND(SUM(CASE WHEN month_goal = 2  THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `2`,
    ROUND(SUM(CASE WHEN month_goal = 3  THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `3`,
    ROUND(SUM(CASE WHEN month_goal = 4  THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `4`,
    ROUND(SUM(CASE WHEN month_goal = 5  THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `5`,
    ROUND(SUM(CASE WHEN month_goal = 6  THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `6`,
    ROUND(SUM(CASE WHEN month_goal = 7  THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `7`,
    ROUND(SUM(CASE WHEN month_goal = 8  THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `8`,
    ROUND(SUM(CASE WHEN month_goal = 9  THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `9`,
    ROUND(SUM(CASE WHEN month_goal = 10 THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `10`,
    ROUND(SUM(CASE WHEN month_goal = 11 THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `11`,
    ROUND(SUM(CASE WHEN month_goal = 12 THEN sales_units_next_year_goal_nonbulk ELSE 0 END), 0) AS `12`,
    ROUND(SUM(sales_units_next_year_goal_nonbulk), 0) AS Grand_Total
FROM sales_model_2027
GROUP BY type_goal WITH ROLLUP
...
SELECT
    ...
    "Effective Price" AS metric,
    ROUND(
        SUM(CASE WHEN month_goal = 1 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 1 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 1 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `1`,
    ROUND(
        SUM(CASE WHEN month_goal = 2 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 2 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 2 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `2`,
    ROUND(
        SUM(CASE WHEN month_goal = 3 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 3 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 3 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `3`,
    ROUND(
        SUM(CASE WHEN month_goal = 4 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 4 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 4 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `4`,
    ROUND(
        SUM(CASE WHEN month_goal = 5 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 5 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 5 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `5`,
    ROUND(
        SUM(CASE WHEN month_goal = 6 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 6 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 6 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `6`,
    ROUND(
        SUM(CASE WHEN month_goal = 7 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 7 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 7 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `7`,
    ROUND(
        SUM(CASE WHEN month_goal = 8 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 8 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 8 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `8`,
    ROUND(
        SUM(CASE WHEN month_goal = 9 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 9 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 9 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `9`,
    ROUND(
        SUM(CASE WHEN month_goal = 10 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 10 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 10 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `10`,
    ROUND(
        SUM(CASE WHEN month_goal = 11 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 11 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 11 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `11`,
    ROUND(
        SUM(CASE WHEN month_goal = 12 THEN sales_rev_next_year_goal_nonbulk ELSE 0 END)
        /
        NULLIF(
            SUM(CASE WHEN month_goal = 12 THEN sales_units_next_year_goal_nonbulk ELSE 0 END)
            - SUM(CASE WHEN month_goal = 12 THEN sales_units_next_year_goal_post_race ELSE 0 END)
        , 0)
    , 2) AS `12`,
    ROUND(
        SUM(sales_rev_next_year_goal_nonbulk)
        /
        NULLIF(
            SUM(sales_units_next_year_goal_nonbulk)
            - SUM(sales_units_next_year_goal_post_race)
        , 0)
    , 2) AS Grand_Total
FROM sales_model_2027
